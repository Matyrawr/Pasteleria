...
const Cart = () => {
  const { cartItems, removeFromCart, updateQuantity, getSubtotal, getDescuento, getTotal, clearCart } = useCart();
        ...
        {cartItems.length === 0 ? (
          ...
          <>
            <div id="items-carrito">
              {cartItems.map(item => {
                const precioUnitario = parseFloat(item.producto.precio.replace(/\./g, '').replace(' CLP', ''));
                  ...
                  <div key={item.producto.codigo} className="item-carrito">
                    <div className="item-info">
                      ...
                      {item.mensajePersonalizado && (
                        <p><strong>Mensaje personalizado:</strong> {item.mensajePersonalizado}</p>
                      )}
                    </div>
                    ...
                    <div className="item-controls">
                      <div className="cantidad-control">
                        ...
                        <input
                          ...
                          min="1"
                          value={item.cantidad}
                          onChange={(e) => handleQuantityChange(item.producto.codigo, e.target.value)}
                        />
                      </div>
                      <p className="precio-total"><strong>Total:</strong> {formatPrice(precioTotal)}</p>
                      <button
            ...
            <div className="carrito-total">
              ...
              {getDescuento() > 0 && (
                <p><strong>Descuento ({usuarioActivo?.descuento}%):</strong> -{formatPrice(getDescuento())}</p>
              )}
              ...
              <div className="carrito-acciones">
                ...
                <button
                  ...
                  onClick={() => {
                    if (usuarioActivo) {
                      alert(`Pago simulado exitoso. Total pagado: ${formatPrice(getTotal())}`);
                      clearCart();
                    }
                  }}
                >
